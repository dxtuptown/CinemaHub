    ...
    public class ShowtimeController : Controller
    {
        ...
        public async Task<IActionResult> Create(ShowtimeVM showtimeVM)
        {
            if (ModelState.IsValid)
            {
                ...
                //Xác định các suất chiếu đã có trong cùng phòng và cùng ngày để kiểm tra xem có xung đột thời gian với suất chiếu mới không.
                ...
                {
                    //Nếu phát hiện xung đột, hệ thống sẽ thông báo lỗi và hiển thị lại trang tạo với các thông tin đã nhập,
                    //đồng thời lấy lại danh sách rạp chiếu và phim từ cơ sở dữ liệu để hiển thị trên giao diện.
                    var cinemas = await _unitOfWork.Cinema.GetAllAsync();
                    var movies = await _unitOfWork.Movie.GetAllAsync();
